...
class Plugin(ERPPlugin):
    """Ecommerce Plugin Implementation"""
    ...
    def description(self) -> str:
        return "Ecommerce module for ERP-Core"
    ...
    def register_routes(self, app: FastAPI, prefix: str = "/api/v1") -> None:
        """Register FastAPI routes for ecommerce."""
        router = APIRouter(prefix=prefix, tags=["Ecommerce"])
        
        @router.get("/")
        async def list_items():
            return {"message": "Ecommerce API", "status": "active"}
    ...
    def register_models(self) -> List:
        """Return SQLAlchemy models for ecommerce."""
        # Define models here
        # Example:
        # class Order(Base):
