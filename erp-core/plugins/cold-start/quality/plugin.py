...
class Plugin(ERPPlugin):
    ...
    def description(self) -> str:
        return "Quality module for ERP-Core"
    ...
    def register_routes(self, app: FastAPI, prefix: str = "/api/v1") -> None:
        """Register FastAPI routes for quality."""
        router = APIRouter(prefix=prefix, tags=["Quality"])
        
        @router.get("/")
        async def list_items():
            return {"message": "Quality API", "status": "active"}
    ...
    def register_models(self) -> List:
        """Return SQLAlchemy models for quality."""
        # Define models here
        # Example:
        # class Order(Base):
