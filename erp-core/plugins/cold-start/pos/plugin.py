...
class Plugin(ERPPlugin):
    ...
    def description(self) -> str:
        return "Pos module for ERP-Core"
    ...
    def register_routes(self, app: FastAPI, prefix: str = "/api/v1") -> None:
        """Register FastAPI routes for pos."""
        router = APIRouter(prefix=prefix, tags=["Pos"])
        
        @router.get("/")
        async def list_items():
            return {"message": "Pos API", "status": "active"}
    ...
    def register_models(self) -> List:
        """Return SQLAlchemy models for pos."""
        # Define models here
        # Example:
        # class Order(Base):
