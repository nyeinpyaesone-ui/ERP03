...
class Plugin(ERPPlugin):
    ...
    def description(self) -> str:
        return "Mrp module for ERP-Core"
    ...
    def register_routes(self, app: FastAPI, prefix: str = "/api/v1") -> None:
        """Register FastAPI routes for mrp."""
        router = APIRouter(prefix=prefix, tags=["Mrp"])
        
        @router.get("/")
        async def list_items():
            return {"message": "Mrp API", "status": "active"}
    ...
    def register_models(self) -> List:
        """Return SQLAlchemy models for mrp."""
        # Define models here
        # Example:
        # class Order(Base):
