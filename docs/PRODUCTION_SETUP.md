...
```bash
# Pull images
docker pull powerrangeranikg/erpo3-backend:latest
docker pull powerrangeranikg/erpo3-frontend:latest
...
```bash
# Database backup
docker-compose exec postgres pg_dump -U erpo3 erpo3-database > backup_$(date +%Y%m%d).sql

# Automated backup (add to crontab)
0 2 * * * cd /opt/erpo3 && ./scripts/backup.sh /backups
```
