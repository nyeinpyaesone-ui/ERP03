...
1. **Backup Data**
   ```bash
   kubectl get secrets -n erpo3 -o yaml > secrets-backup.yaml
   kubectl get configmaps -n erpo3 -o yaml > config-backup.yaml
   ```

2. **Update Redis Deployment**
   ```bash
   kubectl apply -f infra/k8s/base/redis.yaml
   kubectl rollout restart statefulset/erpo3-redis -n erpo3
   ```
