...
resource "aws_subnet" "public" {
  ...
  tags = {
    Name = "erp-${var.environment}-public-${count.index}"
    Type = "public"
  }
}
...
resource "aws_subnet" "private" {
  ...
  availability_zone = var.availability_zones[count.index]
  tags = {
    Name = "erp-${var.environment}-private-${count.index}"
    Type = "private"
  }
}
...
resource "aws_eip" "nat" {
  ...
  domain = "vpc"
  tags = {
    Name = "erp-${var.environment}-nat-eip-${count.index}"
  }
}
...
resource "aws_nat_gateway" "main" {
  ...
  tags = {
    Name = "erp-${var.environment}-nat-${count.index}"
  }
}
...
resource "aws_route_table" "public" {
  ...
  tags = {
    Name = "erp-${var.environment}-public-rt"
  }
}
...
resource "aws_route_table" "private" {
  ...
  tags = {
    Name = "erp-${var.environment}-private-rt-${count.index}"
  }
}
...
resource "aws_route_table" "private_single" {
  ...
  route {
    cidr_block     = "0.0.0.0/0"
    nat_gateway_id = aws_nat_gateway.nat_single[0].id
  }
  tags = {
    Name = "erp-${var.environment}-private-rt"
  }
}
